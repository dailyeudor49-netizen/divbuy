    ...
    <div className="bg-white">
      ...
      <section className="border-b sticky top-16 md:top-18 z-30 bg-white">
        <div className="max-w-7xl mx-auto px-4 py-3 overflow-x-auto">
          <div className="flex gap-2">
            {cats.map((c) => (
              <a key={c.id} href={`#${c.id}`} className="px-3 py-1.5 text-sm bg-gray-100 rounded-full hover:bg-emerald-100 hover:text-emerald-700 whitespace-nowrap">
                {c.name}
              </a>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 space-y-14">
          {cats.map((c, idx) => (
            <div key={c.id} id={c.id} className="scroll-mt-28">
              <div className="mb-5">
                <span className="text-emerald-600 text-sm">{String(idx + 1).padStart(2, "0")}</span>
                <h2 className="text-2xl font-bold text-gray-900">{c.name}</h2>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                {c.items.map((item, i) => (
                  <div key={i} className="p-3 bg-gray-50 rounded-lg hover:bg-emerald-50 transition">
      ...
      <section className="bg-gray-900 py-12">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <Link href="/contact" className="px-6 py-2.5 bg-emerald-600 text-white font-medium rounded-lg hover:bg-emerald-500">
            Get Quote
          </Link>
        </div>
      </section>
